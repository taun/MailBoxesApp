...
- (BOOL)importFileAtPath:(NSString *)filePath attributes:(NSMutableDictionary *)spotlightData error:(NSError **)error {
    ...
    if (!oid) {
        NSLog(@"%@:%@ to find object id from path %@", NSStringFromClass([self class]), NSStringFromSelector(_cmd), filePath);
        return NO;
    }
...
- (NSManagedObjectModel *)managedObjectModel {
	...
	if (!managedObjectModel) {
		...
		if (!managedObjectModel) {
			NSLog(@"%@:%@ unable to load model at URL %@", NSStringFromClass([self class]), NSStringFromSelector(_cmd), modelURL);
			return nil;
		}
...
- (NSPersistentStoreCoordinator *) persistentStoreCoordinator {
    ...
    if (![persistentStoreCoordinator addPersistentStoreWithType:YOUR_STORE_TYPE 
										...
										options:nil 
										error:&error]){
        NSLog(@"%@:%@ unable to add persistent store coordinator - %@", NSStringFromClass([self class]), NSStringFromSelector(_cmd), error);
    }    
...
- (NSManagedObjectContext *) managedObjectContext {
	...
	if (!coordinator) {
        NSLog(@"%@:%@ unable to get persistent store coordinator", NSStringFromClass([self class]), NSStringFromSelector(_cmd));
		return nil;
	}
